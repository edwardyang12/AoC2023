...
expand_map = {
    'J': np.array([[np.inf, 1, np.inf], [1, 1, np.inf], [np.inf, np.inf, np.inf]]),
    '|': np.array([[np.inf, 1, np.inf], [np.inf, 1, np.inf], [np.inf, 1, np.inf]]),
    '-': np.array([[np.inf, np.inf, np.inf], [1, 1, 1], [np.inf, np.inf, np.inf]]),
    'L': np.array([[np.inf, 1, np.inf], [np.inf, 1, 1], [np.inf, np.inf, np.inf]]),
    '7': np.array([[np.inf, np.inf, np.inf], [1, 1, np.inf], [np.inf, 1, np.inf]]),
    'F': np.array([[np.inf, np.inf, np.inf], [np.inf, 1, 1], [np.inf, 1, np.inf]])
}

# DFS from starting point on 2D array to get full tunnel
# expand each tile to 3x3 and then flood fill 
# all impossible states should maintain all inf in 3x3 grid
# || -> 0 1 0 0 1 0 meaning there's space
...
# turn start into a normal tile 
if start[0]+1 < len(grid_string) and (grid_string[start[0]+1][start[1]]== '|' or grid_string[start[0]+1][start[1]]== 'L' or grid_string[start[0]+1][start[1]]== 'J'):
    if start[1]+1 < len(grid_string[0]) and (grid_string[start[0]][start[1]+1]== '-' or grid_string[start[0]][start[1]+1]== '7' or grid_string[start[0]][start[1]+1]== 'J'):
        grid_string[start[0]][start[1]] = 'F'
    elif start[1]-1 >=0 and (grid_string[start[0]][start[1]-1]== '-' or grid_string[start[0]][start[1]-1]== 'L' or grid_string[start[0]][start[1]-1]== 'F'):
        grid_string[start[0]][start[1]] = '7'
elif start[0]-1 >=0 and (grid_string[start[0]-1][start[1]]== '|' or grid_string[start[0]-1][start[1]]== '7' or grid_string[start[0]-1][start[1]]== 'F'):
    if start[1]+1 < len(grid_string[0]) and (grid_string[start[0]][start[1]+1]== '-' or grid_string[start[0]][start[1]+1]== '7' or grid_string[start[0]][start[1]+1]== 'J'):
        grid_string[start[0]][start[1]] = 'L'
    elif start[1]-1 >=0 and (grid_string[start[0]][start[1]-1]== '-' or grid_string[start[0]][start[1]-1]== 'L' or grid_string[start[0]][start[1]-1]== 'F'):
        grid_string[start[0]][start[1]] = 'J'

# expansion
expand_grid_result = np.full((len(grid_result)*3, len(grid_result[0]*3)), np.inf)
for x in range(len(grid_result)):
    for y in range(len(grid_result[0])):
        if grid_result[x][y] == 1:
            expansion = expand_map[grid_string[x][y]]
            center_x, center_y = 3*x + 1, 3*y + 1
            expand_grid_result[center_x-1:center_x+2, center_y-1:center_y+2] = expansion 
...
while stack:
    ...
    for dir in valid_dir:
        ...
        if row < 0 or row >= len(expand_grid_result):
            continue 
        elif col < 0 or col >= len(expand_grid_result[0]):
            continue 
        if expand_grid_result[row][col] == math.inf:
            expand_grid_result[row][col] = 0
...
for x in range(len(grid_result)):
    for y in range(len(grid_result[0])):
        center_x, center_y = 3*x + 1, 3*y + 1
        if np.isinf(expand_grid_result[center_x-1:center_x+2, center_y-1:center_y+2]).sum() == 9:
            count += 1
print(count)